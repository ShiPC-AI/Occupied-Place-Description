...
// raster map
// std::vector<std::vector<bool>> _raster_map;
// void makeRasterMap(const PointCloudPtr& cloud_in) {
//     pcl::PointXYZ pt_max, pt_min;
//     pcl::getMinMax3D(*cloud_in, pt_min, pt_max);
//     float offset_x = -pt_min.x;
//     float offset_y = -pt_min.y;

//     size_t cols_raster = (pt_max.x - pt_min.x) / PARA_RASTER_SIZE; // 1.0
//     size_t rows_raster = (pt_max.y - pt_min.y) / PARA_RASTER_SIZE; // 1.0
//     _raster_map.resize(rows_raster);
//     for (int i = 0; i < _raster_map.size(); ++i) {
//         _raster_map[i] = std::vector<bool>(cols_raster, false);
//     }

//     Eigen::Matrix4f trans_affine= Eigen::Matrix4f::Identity();
//     trans_affine(0, 0) = 1.0 / PARA_RASTER_SIZE;
//     trans_affine(1, 1) = 1.0 / PARA_RASTER_SIZE;
//     trans_affine(0, 3) = offset_x / PARA_RASTER_SIZE;
//     trans_affine(1, 3) = offset_y / PARA_RASTER_SIZE;
//     PointCloudPtr transed_cloud(new PointCloud());
//     pcl::transformPointCloud(*cloud_in, *transed_cloud, trans_affine);

//     for (int i = 0; i < transed_cloud->size(); ++i) {
//         const pcl::PointXYZ& pt = transed_cloud->points[i];
//         int c = int(pt.x);
//         int r = int(pt.y);
//         if (c < 0 || c >= cols_raster || r < 0 || r >= rows_raster) {
//             continue;
//         }
//         _raster_map[r][c] = true;
//     }
// }
...
int main(int argc, char** argv) {
    ...
    // subscriber
    ros::Subscriber sub_scan = nh.subscribe("/lidar_points", 1000, recieveCloud);
    ...
    while (ros::ok()) {
        ...
        if ((ros::Time::now() - last_publish_time).toSec() > 6 && _cloud_buf.empty() && _pose_buf.empty()) {
            if (!_pass_map->empty() && !_candidate_pts->empty() && !_raw_map->empty()) {
                ...
                // candidates
                voxelSampleCloud(_candidate_pts, _candidate_pts, PARA_GRID_SIZE_GROUND, PARA_GRID_SIZE_GROUND, PARA_GRID_SIZE_GROUND);
                ...
                // pass_map
                if (_pass_map->size() > 200000) {
                    randomSampleCloud(_pass_map, _pass_map, 200000);
                }
        ...
        removeCloseAndFarPts(*_scan_cloud, *_scan_cloud, 5.0, 80.0);
